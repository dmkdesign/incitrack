...
<?php 
	...
	<div class="row">
		...
		if ($field->widgetEdit($profile)) {
		 ...
		 elseif ($field->range) {
			if(($userType==User::TYPE_SUPERVISOR)&&($field->varname=='userType'))
			{
				echo CHtml::activeTextField($profile,$field->varname,array('readonly'=>'readonly', 'size'=>60,'maxlength'=>(($field->field_size)?$field->field_size:255)));
			}
			else{
			...
			$htmlOptions=array('size'=>60,'maxlength'=>(($field->field_size)?$field->field_size:255));
			if(($userType==User::TYPE_SUPERVISOR)&&($field->varname=='company'))
			{$htmlOptions['readonly']='readonly';}
			echo CHtml::activeTextField($profile,$field->varname,$htmlOptions);
		}
		 ?>
