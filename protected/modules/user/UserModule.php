...
class UserModule extends CWebModule
{
	...
	public $returnLogoutUrl = array("/site/Landing");
	...
	/**
	 * Return suuprviors.these user types can modify and add report creators (normal) user types and modify company info.
	 * @return array superusers names
	 */
	public static function getSupervisors() {
			$supervisors = User::model()->with(array('profile'=>array('condition'=>'userType='.User::TYPE_SUPERVISOR)))->findAll();
			$return_name = array();
			foreach ($supervisors as $supervisor){
				array_push($return_name,$supervisor->username);
...
*send mail with PEAR
*/
 public static function sendMailPear($email, $subject, $message)
 {
 $from = "GridBid <info@gridbid.it>";
 $to = $email;
 $body = $message;
 ...
 if (PEAR::isError($mail)) {
   Yii::app()->user->setFlash('error', "An email was trying to be sent to ".$email." but failed.  Don't fear, your auction has been started!");
  }
  else
  {
