...
class QuestionController extends Controller
{
	...
	public function actionView()
	{
		...
		$cs->registerScript(
				...
				$("#choicesdiv").show("slow");}',
				CClientScript::POS_END
		);
		$criteria=new CDbCriteria(array(
				'condition'=>'question='.$model->id
					
		));
		
		$dataProvider=new CActiveDataProvider('Choices', array(
				'criteria'=>$criteria,
		));
		$this->render('view',array(
			'model'=>$model,'choices'=>$choices,'dataProvider'=>$dataProvider
		));
	}
	...
	public function actionCreate($pageid)
	{
		...
		if(isset($_POST['Question']))
		{
			...
			//decode field_type as 2 non-innoDB field types now exist(RADIO and SELECT)
			if($model->field_type=='SELECT'||$model->field_type=='RADIO')
			{
				$field_type= 'VARCHAR';
			}
			else {
			...
			if($model->validate()) {
				$sql = 'ALTER TABLE '.Report::model()->tableName().' ADD `'.$model->varname.'` ';
				$sql .= $this->fieldType($field_type);
				if (
				...
				{
					if($pageid)
						$this->redirect(array('page/view','id'=>$pageid));
				}
			}
		}
	...
	public function actionDelete()
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			if ($scheme=='CSqliteSchema') {
				$attr = Question::model()->attributes;
				...
				try
				{
				    ...
				    $connection->createCommand(
				    	"CREATE TEMPORARY TABLE ".Question::model()->tableName()."_backup (".implode(',',$attr).")"
				    )->execute();
				    
				    $connection->createCommand(
				    	"INSERT INTO ".Question::model()->tableName()."_backup SELECT ".implode(',',$attr)." FROM ".Question::model()->tableName()
				    )->execute();
				    
				    $connection->createCommand(
				    	"DROP TABLE ".Question::model()->tableName()
				    )->execute();
				    
				    $connection->createCommand(
				    	"CREATE TABLE ".Question::model()->tableName()." (".implode(',',$attr).")"
				    )->execute();
				    
				    $connection->createCommand(
				    	"INSERT INTO ".Question::model()->tableName()." SELECT ".implode(',',$attr)." FROM ".Question::model()->tableName()."_backup"
				    )->execute();
				    
				    $connection->createCommand(
				    	"DROP TABLE ".Question::model()->tableName()."_backup"
				    )->execute();
				...
				//there was an error somewhere in the previous code, the following mess was the result.  Changed order of SQL commands and it seemed to work.
				$varname= $model->varname;
				$tableName = Question::model()->tableName();
				$question=Question::model()->find(array(
						'select'=>'id',
						'condition'=>'varname=:varname',
						'params'=>array(':varname'=>$varname),
				));
				$varId=$question->id;
				$sql ='DELETE FROM incidents.'. $tableName.' WHERE '.$tableName.'.id = '.$varId;
				$model->dbConnection->createCommand($sql)->execute();
				$sql = 'ALTER TABLE '.Report::model()->tableName().' DROP `'.$model->varname.'`;';
						//'DELETE FROM '.Question::model()->tableName().' WHERE `varname` =`'.$model->varname.'`;';
				$model->dbConnection->createCommand($sql)->execute(); 
	...
	public function actionAdmin()
	{
		$dataProvider=new CActiveDataProvider('Question', array(
			'pagination'=>array(
				'pageSize'=>Yii::app()->controller->module->page_size,
			),
			'sort'=>array(
