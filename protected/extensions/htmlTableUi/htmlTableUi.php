...
class HtmlTableUi extends CWidget
{
	 ...
	 * (optional)
	*
	* This parameter specifies where images are loaded from relative to webroot.
	*/
	public $imagePath='';
	// Public Attributes----------->
			...
			echo CHtml::openTag('div',array('class'=>'body ui-widget-content ui-corner-bottom','style'=>'display:'.$display.';'));
				echo CHtml::openTag('table',array('id'=>$this->getId().'-body-table','class'=>'body-table'));
					...
					if(isset($this->columns)){
						...
						$columnData = CHtml::tag('div',array('class'=>'column-name'),'<span>Image</span>').CHtml::tag('div',array('class'=>'column-sort-image'));
						$columnData = CHtml::tag('div',array('class'=>'column-data'),$columnData);
						$header .= CHtml::tag('th',array('class'=>'unsorted ui-widget-header'),$columnData);
						   	...
						   	echo CHtml::openTag('tr',$rowOptions);							
								...
								if(count($row)){
									...
									foreach($row as $value){
										if(strpos(strtoupper($value),'.PNG')||strpos(strtoupper($value),'.JPG')||strpos(strtoupper($value),'.BMP'))
										{
										$picVal = $value;
										}
										echo CHtml::tag('td',array(),$value);
										if($count == count($row)-1){
											echo "<td><img src='".$this->imagePath."/".$picVal."' width='150px'/></td>" ;
										}
										$count++;
									}
								}
